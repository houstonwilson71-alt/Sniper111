...
async fn main() -> Result<()> {
    ...
    let _sol_handle = tokio::spawn(listener_solana::run(cfg.clone(), event_tx.clone()));
    let _bsc_handle = tokio::spawn(listener_bsc::run(cfg.clone(), event_tx.clone()));
    ...
    let _sol_exec = tokio::spawn(executor_solana::run(cfg.clone(), event_tx.clone(), sol_cmd_rx));
    let _bsc_exec = tokio::spawn(executor_bsc::run(cfg.clone(), event_tx.clone()));
    let _pos_monitor = tokio::spawn(position_monitor::run(cfg.clone(), event_tx.clone(), cmd_rx));

    // Safety filter loop: receives detected tokens, runs checks, emits buy signals.
    let _filter_handle = tokio::spawn(filter::run(cfg.clone(), event_tx.clone()));

    info!("Engine tasks spawned; waiting for shutdown signal");
